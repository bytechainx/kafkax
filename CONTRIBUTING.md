...
- `Cargo.toml` 的 `documentation` 指向 `https://github.com/bytechainx/kafkax#readme`。
...
- 错误类型：`thiserror` 枚举 + `#[non_exhaustive]` + `pub type KafkaResult<T>` 别名。
...
- 配置来源优先级固定为：默认值 → TOML（`from_toml`）→ 环境变量（`from_env`）→ builder；
  新增 env 键必须同时定义 `ENV_*` 常量，禁止调用方硬编码字符串。
- SASL 凭据只能经环境变量或 builder 注入，TOML 禁止出现；`Debug` 输出必须脱敏。
- 不做隐式重试或自动重连：重试判定统一走 `KafkaError::is_retryable()`，退避由调用方实现。
...
- [ ] 未向 TOML 配置引入 `sasl_username` / `sasl_password` 字段
